...
#Load 
df <- read.csv("TestData_Rouse.csv")
...
stats_mean_flow <- df %>%
  ...
  summarise(
    ...
    max= max(Rouse),
    mean = mean(Rouse),
    p5 = quantile(Rouse, 0.05),
    q25 = quantile(Rouse, 0.25),
    q50 = median(Rouse),
    q75 = quantile(Rouse, 0.75),
    p95 = quantile(Rouse, 0.95)
  )

stats_mean_flow$n <- df %>%
count(Flow.Type) 

ggplot(df, aes(x=Rouse, fill=Flow.Type)) + 
  ...
  labs(title="Test Data Rouse Numbers: Mean MaTCH Density Distributions",
       x="Rouse Numbers",
       y="Density") +
  theme_minimal()
...
df_10 <- read.csv("Test_Rouse_10th.csv")
...
ggplot(df_10, aes(x=Rouse, fill=Flow.Type)) + 
  geom_density(alpha=0.7) +
  facet_wrap(~Flow.Type, scales="free_y", ncol = 1) +
  geom_vline(xintercept = c(-2.5, 0.8, -0.8, 2.5, 7.5), linetype = "dashed", color = "darkblue") +
  geom_vline(xintercept = c(-1.8, -0.6), linetype = "dashed", color = "gray") +
  scale_x_continuous(limits = c(-5, 7.5), breaks = seq(-5, 7.5, by = 2.5)) +  # Set x-axis limits and breaks +
  labs(title="Test Data Rouse Numbers: 10th percentile MaTCH Density Distributions",
       x="Rouse Numbers",
       y="Density") +
  theme_minimal()
...
df_90 <- read.csv("Test_Rouse_90th.csv")
...
ggplot(df_90, aes(x=Rouse, fill=Flow.Type)) + 
  geom_density(alpha=0.7) +
  facet_wrap(~Flow.Type, scales="free_y", ncol = 1) +
  geom_vline(xintercept = c(-2.5, 0.8, -0.8, 2.5, 7.5), linetype = "dashed", color = "darkblue") +
  geom_vline(xintercept = c(-1.8, -0.6), linetype = "dashed", color = "gray") +
  scale_x_continuous(limits = c(-5, 7.5), breaks = seq(-5, 7.5, by = 2.5)) +  # Set x-axis limits and breaks +
  labs(title="Test Data Rouse Numbers: 90th percentile MaTCH Density Distributions",
       x="Rouse Numbers",
       y="Density") +
  theme_minimal()
...
#Load 
df_sum <- read.csv("Test_Rouse_FTIR_short_summary.csv")
df_sum <- df_sum %>% 
  filter(Rouse_mean != "NA") %>%
  filter(Rouse_10 != "NA") %>%
  filter(Rouse_90 != "NA") %>% 
  filter(Event != "SF-6")
...
df_percent_Cowger <- df_sum %>%
  group_by(Flow.Type) %>%
  summarise(
    Percent_Mean_Surface = calc_percentage(Rouse_mean, ranges[["surface load"]]),
    Percent_Mean_RiseSus = calc_percentage(Rouse_mean, ranges[["rising suspended load"]]),
    Percent_Mean_Wash = calc_percentage(Rouse_mean, ranges[["wash load"]]),
    Percent_Mean_SetSus = calc_percentage(Rouse_mean, ranges[["settling suspended load"]]),
    Percent_Mean_Bed = calc_percentage(Rouse_mean, ranges[["bed load"]]),
    Percent_Mean_Immob = calc_percentage(Rouse_mean, ranges[["immobile"]]),
    Percent_10_Surface = calc_percentage(Rouse_10, ranges[["surface load"]]),
    Percent_10_RiseSus= calc_percentage(Rouse_10, ranges[["rising suspended load"]]),
    Percent_10_Wash = calc_percentage(Rouse_10, ranges[["wash load"]]),
    Percent_10_SetSus = calc_percentage(Rouse_10, ranges[["settling suspended load"]]),
    Percent_10_Bed = calc_percentage(Rouse_10, ranges[["bed load"]]),
    Percent_10_Immob = calc_percentage(Rouse_10, ranges[["immobile"]]),
    Percent_90_Surface = calc_percentage(Rouse_90, ranges[["surface load"]]),
    Percent_90_RiseSus = calc_percentage(Rouse_90, ranges[["rising suspended load"]]),
    Percent_90_Wash = calc_percentage(Rouse_90, ranges[["wash load"]]),
    Percent_90_SetSus = calc_percentage(Rouse_90, ranges[["settling suspended load"]]),
    Percent_90_Bed = calc_percentage(Rouse_90, ranges[["bed load"]]),
    Percent_90_Immob = calc_percentage(Rouse_90, ranges[["immobile"]]),
  )
...
df_long <- df_sum %>%
  pivot_longer(cols = c(Rouse_mean, Rouse_10, Rouse_90),
               names_to = "Scenario",
               values_to = "Rouse_value")

library(ggplot2)

ggplot(df_long, aes(x=Rouse_value, fill=Scenario)) + 
  geom_density(alpha=0.5) +  # Adjust alpha for transparency
  facet_wrap(~Flow.Type, scales="free_y", ncol = 1) +
  geom_vline(xintercept = c(-2.5, 0.8, -0.8, 2.5, 7.5), linetype = "dashed", color = "darkblue") +
  geom_vline(xintercept = c(-1.8, -0.6), linetype = "dashed", color = "gray") +
  scale_x_continuous(limits = c(-5, 7.5), breaks = seq(-5, 7.5, by = 2.5)) +
  labs(title="Test Data Rouse Numbers: μFTIR subset",
       x="Rouse Numbers",
       y="Density") +
  theme_minimal() +
...
ggplot() + 
  # First scenario
  geom_density(data = df_sum, aes(x = Rouse_mean, fill = "Mean"), alpha = 0.4) +
  
  # Second scenario
  geom_density(data = df_sum, aes(x = Rouse_10, fill = "10th Percentile"), alpha = 0.4) +
  
  # Third scenario
  geom_density(data = df_sum, aes(x = Rouse_90, fill = "90th Percentile"), alpha = 0.4) +
  ...
  labs(title="Test Data Rouse Numbers: μFTIR subset (Fibers and Non-Fibers)",
       x="Rouse Numbers",
       y="Density (%)") +
...
df <- read.csv("Test_Rouse_Mean.csv")
...
ggplot() + 
  ...
  labs(title="Test Data Rouse Numbers: Stormflow μFTIR subset (Fibers and Non-Fibers)",
       x="Rouse Numbers",
       y="Density (%)") +
  theme_minimal() +
  theme(
