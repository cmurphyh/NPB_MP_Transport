...
ggplot(df_long, aes(x=Rouse_adj_value, fill=Scenario)) + 
  ...
  labs(title="SAD Rouse Numbers: Suspected TRWPs",
       x="Rouse Numbers",
       y="Density (%)") +
  theme_minimal() +
...
custom_labels <- c(
  "Inside Thalweg" = "Thalweg (n=1080)",
  "Outside Thalweg_1" = "Nearest Right Bank (n=502)",
  "Outside Thalweg_2" = "Nearest Left Bank (n=478)"
)
...
ggplot() + 
  ...
  scale_x_continuous(limits = c(-5, 7.5), breaks = seq(-5, 7.5, by = 2.5)) +
  scale_y_continuous(limits=c(0,2),breaks = seq(0, 2.0, by = 0.5))+
  labs(title="SAD Rouse Numbers: Stormflow Suspected TRWPs",
       x="Rouse Numbers",
       y="Density (%)") +
  theme_minimal() +
  theme(
  ...
  theme(legend.position="bottom") +
  scale_fill_manual(name = "Particle Density (MaTCH) Assigned", values = c("Mean" = "blue", "10th Percentile" = "red", "90th Percentile" = "green")) +
  geom_text(data = df_percent_Cowger, aes(x = 6, y=0.8, label = paste0("% Washload","\nMean: ", round(Percent_Mean_Wash, 2), "%\n10th: ", round(Percent_10_Wash, 2), "%\n90th: ", round(Percent_90_Wash, 2), "%")), size = 4.5, hjust = 0.5) +
  geom_label(data = df_percent_Cowger, aes(x = 0, y = 1.85, label = "Wash load"), size = 3, color = "black", fill = "white", fontface = "bold") +
  geom_label(data = df_percent_Cowger, aes(x = 1.65, y = 1.65, label = "Settling \nSuspended \nLoad"), size = 3, color = "black", fill = "white", fontface = "bold") +
  geom_label(data = df_percent_Cowger, aes(x = -1.65, y = 1.65, label = "Rising \nSuspended \nLoad"), size = 3, color = "black", fill = "white", fontface = "bold") +
  geom_label(data = df_percent_Cowger, aes(x = -3.75, y = 1.85, label = "Surface Load"), size = 3, color = "black", fill = "white", fontface = "bold") +
  geom_label(data = df_percent_Cowger, aes(x = 5, y = 1.85, label = "Bed Load"), size = 3, color = "black", fill = "white", fontface = "bold") 
###################################
