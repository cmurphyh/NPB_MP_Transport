...
SAD_SF1_NF_5 <-read.csv("SAD-SF-1_NF_5.csv")  #read in database as csv
head(SAD_SF1_NF_5)
class(SAD_SF1_NF_5)
#check data is between 250um and 5 mm
summary(SAD_SF1_NF_5$Feret)


#Plot histogram of frequencies (all intervals)
hist(SAD_SF1_NF_5$Feret, xlab = 'Feret Diameter [mm]', xlim = c(0,5), breaks= seq(0,5,0.25), xaxp=c(0.25,5.25,20),
...
abline(v=0.330,col="red",lwd=2, lty="dotted", 
       text(1.43,60,"net mesh size (330 um)",col = "red"))
title(main ="SAD SF-1 Sample: Size Distribution (Non-Fibers Only)",line=0.5, cex.lab=1)
##############################################################################################
#Creating a Cumulative Frequency Plot (all intervals) --> do this first to use in relative frequency
# declaring data points
data_points <- SAD_SF1_NF_5[,16] # calling up column for Feret Values Only
...
abline(v=0.330,col="red", lty="dotted", lwd=2, text(1.4,100,"net mesh size (330 um)",col = "red"))
title(main ="SAD SF-1 Sample: Size Distribution",line=1, cex.lab=1)
##############################################################################################
##############################################################################################
##############################################################################################
##############################################################################################
...
#Load subtidal Non-Fiber Data with data restricted to 250um to 5 mm (feret diameters)
SAD_SF2_NF_5 <-read.csv("SAD-SF-2_NF_5.csv")  #read in database as csv
head(SAD_SF2_NF_5)
class(SAD_SF2_NF_5)
#check data is between 250um and 5 mm
summary(SAD_SF2_NF_5$Feret)


#Plot histogram of frequencies (all intervals)
hist(SAD_SF2_NF_5$Feret, xlab = 'Feret Diameter [mm]', xlim = c(0,5), breaks= seq(0,5,0.25), xaxp=c(0.25,5.25,20),
     col= 'light blue', main = " ",cex.lab=1.1, cex.axis=1.1, cex.main=1.2,las=1)
title(main ="SAD SF-2:
 Size of Non-Fiber Suspected Microplastics",line=-1, cex.lab=1)
...
title(main ="SAD SF-2 Sample: Size Distribution (Non-Fibers Only)",line=0.5, cex.lab=1)
##############################################################################################
#Creating a Cumulative Frequency Plot (all intervals) --> do this first to use in relative frequency
# declaring data points
data_points <- SAD_SF2_NF_5[,16] # calling up column for Feret Values Only
class(data_points) #check Numeric
# declaring the break points
break_points <- seq(0.25, 5, by=0.25) #Bin sizes
# transforming the data
...
title(main ="SAD SF-2 Sample: Size Distribution",line=1, cex.lab=1)
##############################################################################################
##############################################################################################
##############################################################################################
##############################################################################################
...
#Load subtidal Non-Fiber Data with data restricted to 250um to 5 mm (feret diameters)
SAD_SF4_NF_5 <-read.csv("SAD-SF-4_NF_5.csv")  #read in database as csv
head(SAD_SF4_NF_5)
class(SAD_SF4_NF_5)
#check data is between 250um and 5 mm
summary(SAD_SF4_NF_5$Feret)


#Plot histogram of frequencies (all intervals)
hist(SAD_SF4_NF_5$Feret, xlab = 'Feret Diameter [mm]', xlim = c(0,5), breaks= seq(0,5,0.25), xaxp=c(0.25,5.25,20),
     col= 'light blue', main = " ",cex.lab=1.1, cex.axis=1.1, cex.main=1.2,las=1)
title(main ="SAD SF-4:
 Size of Non-Fiber Suspected Microplastics",line=-1, cex.lab=1)
...
abline(v=0.250,col="red",lwd=2, lty="dotted", 
       text(1.4,62,"net mesh size (250 um)",col = "red"))
title(main ="SAD SF-4 Sample: Size Distribution (Non-Fibers Only)",line=0.5, cex.lab=1)
##############################################################################################
#Creating a Cumulative Frequency Plot (all intervals) --> do this first to use in relative frequency
# declaring data points
data_points <- SAD_SF4_NF_5[,16] # calling up column for Feret Values Only
class(data_points) #check Numeric
# declaring the break points
break_points <- seq(0.25, 5, by=0.25) #Bin sizes
# transforming the data
...
lines(break_points, relcumulative_freq)
abline(v=0.250,col="red", lty="dotted", lwd=2, text(1.35,100,"net mesh size (250 um)",col = "red"))
title(main ="SAD SF-4 Sample: Size Distribution",line=1, cex.lab=1)