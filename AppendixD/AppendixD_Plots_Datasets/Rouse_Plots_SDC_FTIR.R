...
custom_labels <- c(
  "Low Flow" = "Low Flow (n = 244)",
  "Stormflow" = "Stormflow (n = 537)"
)
...
ggplot() + 
  ...
  labs(title="SDC Rouse Numbers: μFTIR subset (Fibers and Non-Fibers)",
       x="Rouse Numbers",
       y="Density (%)") +
...
custom_labels <- c(
  "Inside Thalweg" = "Thalweg (n=190)",
  "Outside Thalweg_1" = "Nearest Right Bank (n=124)"
)
...
ggplot() + 
  ...
  scale_x_continuous(limits = c(-5, 7.5), breaks = seq(-5, 7.5, by = 2.5)) +
  scale_y_continuous(limits=c(0,4.5),breaks = seq(0, 4.5, by = 0.5))+
  labs(title="SDC Rouse Numbers: Stormflow μFTIR subset (Fibers and Non-Fibers)",
       x="Rouse Numbers",
       y="Density (%)") +
  theme_minimal() +
  theme(
